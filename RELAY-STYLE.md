...
This document explains how Relay is written so future Claude instances can contribute consistently.

## Core Philosophy

Relay is a **meta-prompting system** where every file is both implementation and specification. Files teach Claude how to build software systematically. The system optimizes for:

- **Ticket-driven workflow** (fetch → analyze → plan → confirm → execute → verify → sync)
...
### Slash Commands (`commands/relay/*.md`)

```yaml
---
name: relay:command-name
...
Some workflows use domain-specific tags like `<philosophy>`, `<references>`, `<planning_principles>`.
...
- Square brackets: `[Ticket Title]`, `[Description]`
- Curly braces: `{ticket_id}/{slug}`

### References (`relay/references/*.md`)

Typically use outer XML containers related to filename, but structure varies.

Examples:
- `principles.md` → `<principles>...</principles>`
- `checkpoints.md` → `<checkpoint_types>`
...
|------|------------|---------|
| Files | kebab-case | `analyze-ticket.md` |
| Commands | `relay:kebab-case` | `relay:work` |
| XML tags | kebab-case | `<execution_context>` |
| Step names | snake_case | `name="load_ticket_state"` |
| Bash variables | CAPS_UNDERSCORES | `TICKET_ID`, `PLAN_START_TIME` |
...
**Bad one-liner:** "Ticket complete" or "Authentication implemented"
...
- `STATE.md` — Living memory across sessions (active ticket, velocity)
- `ANALYSIS.md` — Ticket requirements mapped to codebase
- `SUMMARY.md` — Execution results, machine-readable frontmatter
...
- Ticket ID in every commit message
...
**Visual patterns:** `relay/references/ui-brand.md`
...
Quick mode provides Relay guarantees for ad-hoc tasks without full ticket overhead.
...
- Task is tracked in your ticket system
...
Unlike the full ticket workflow, quick mode orchestration is inline in the command file — no separate workflow. The simplified flow:

1. Validate config.json exists (project active)
...
- RED: `test({ticket_id}): add failing test for [feature]`
- GREEN: `feat({ticket_id}): implement [feature]`
- REFACTOR: `refactor({ticket_id}): clean up [feature]`
...
6. **Ticket-driven workflow** — fetch, analyze, plan, execute, verify, sync
...
10. **Atomic commits** — ticket ID in every commit
...
14. **TDD gets dedicated plans** — cycle too heavy to embed
