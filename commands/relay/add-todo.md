...
<objective>
Capture an idea, task, or issue that surfaces during a Relay session as a structured todo for later work.
...
```bash
mkdir -p .relay/todos/pending .relay/todos/done
```
</step>

<step name="check_existing_areas">
```bash
ls .relay/todos/pending/*.md 2>/dev/null | xargs -I {} grep "^area:" {} 2>/dev/null | cut -d' ' -f2 | sort -u
```
...
- `/relay:add-todo Add auth token refresh` → title = "Add auth token refresh"
...
```bash
grep -l -i "[key words from title]" .relay/todos/pending/*.md 2>/dev/null
```
...
Write to `.relay/todos/pending/${date_prefix}-${slug}.md`:
...
If `.relay/STATE.md` exists:

1. Count todos: `ls .relay/todos/pending/*.md 2>/dev/null | wc -l`
...
<step name="confirm">
```
Todo saved: .relay/todos/pending/[filename]
...
2. Add another todo
3. View all todos (/relay:check-todos)
```
</step>
...
<output>
- `.relay/todos/pending/[date]-[slug].md`
- Updated `.relay/STATE.md` (if exists)
</output>
